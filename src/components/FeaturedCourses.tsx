    ...
    <section id="courses" className="bg-white py-20 md:py-32">
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map((course) => (
            <div key={course.id} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full group border border-gray-100">
              <Link to={`/courses/${course.id}`} className="block flex-1 flex flex-col">
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <div className="absolute top-4 right-4 bg-yutime-gold text-yutime-indigo px-4 py-2 rounded-full text-sm font-bold shadow-md">
                    {course.level}
                  </div>
                </div>
                <div className="flex-1 flex flex-col p-6">
                  <div className="mb-4">
                    <span className="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-medium">{course.category}</span>
                  </div>
                  <h3 className="text-xl font-bold mb-3 text-gray-900 leading-tight">{course.title}</h3>
                  <p className="text-gray-600 text-sm mb-6 flex-1 leading-relaxed">{course.description}</p>
                  <div className="space-y-4">
                    <p className="text-yutime-blue text-2xl font-bold">HKD {course.price}</p>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500 font-medium">{course.totalTime}</span>
                      <Button 
                        className="bg-yutime-indigo hover:bg-yutime-indigo/90 text-white px-6 py-2 rounded-full font-medium"
                        onClick={(e) => {
                          e.preventDefault();
