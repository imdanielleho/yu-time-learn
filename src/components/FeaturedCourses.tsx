...
const FeaturedCourses = () => {
  ...
  const CourseCard = ({
    ...
    <div className="bg-white border-2 border-yutime-neutral/20 rounded-2xl overflow-hidden flex flex-col h-full group hover:border-yutime-secondary/40 hover:shadow-wellness focus-within:ring-2 focus-within:ring-yutime-secondary/20 transition-all duration-300 shadow-soft">
      <Link to={`/courses/${course.id}`} className="block flex-1 flex flex-col">
        ...
        <div className="flex-1 flex flex-col p-5 md:p-6">
          <div className="mb-3">
            <span className="bg-yutime-secondary/15 text-yutime-secondary px-3 py-1.5 rounded-full text-sm font-medium border border-yutime-secondary/20">
              {course.category}
            </span>
          </div>
          <h3 className="text-xl font-semibold mb-3 text-yutime-primary group-hover:text-yutime-secondary transition-colors duration-300 leading-snug">
            {course.title}
          </h3>
          ...
          <div className="mt-auto pt-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            ...
            <Button className="bg-yutime-secondary hover:bg-yutime-secondary/90 text-white text-sm px-6 py-3 w-full md:w-auto rounded-xl font-medium transition-all duration-300 transform hover:scale-[1.02] shadow-gentle hover:shadow-warm border border-yutime-secondary/20">
              開始試看
            </Button>
          </div>
        </div>
  ...
  return (
    <section id="courses" className="bg-white py-12 md:py-16 lg:py-20 border-t border-yutime-neutral/10">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {isMobile ? (
            <div className="w-full max-w-sm mx-auto sm:max-w-none">
              ...
              {/* Enhanced mobile carousel indicators matching HeroCarousel style */}
