      ...
      <div className="container">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {courses.map((course) => (
              <div key={course.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 group">
                <Link to={`/courses/${course.id}`} className="block">
                  <div className="relative h-48 overflow-hidden">
                    ...
                    />
                    <div className="absolute top-3 left-3 bg-white text-gray-900 px-3 py-1 rounded-full text-sm font-medium">
                      {course.level}
                    </div>
                  </div>
                  <div className="p-6">
                    <div className="mb-3">
                      <span className="text-yutime-blue text-sm font-medium">{course.category}</span>
                    </div>
                    <h3 className="text-xl font-semibold mb-3 text-gray-900">{course.title}</h3>
                    <p className="text-gray-600 text-sm mb-4 leading-relaxed">{course.description}</p>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-2xl font-bold text-gray-900">HKD {course.price}</span>
                    </div>
                    <div className="flex flex-col gap-3">
                      <span className="text-sm text-gray-500">{course.totalTime}</span>
                      <Button 
                        className="bg-yutime-indigo hover:bg-yutime-indigo/90 text-white w-full"
                        onClick={(e) => {
                          e.preventDefault();
