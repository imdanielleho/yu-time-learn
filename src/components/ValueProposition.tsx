...
const ValueProposition = () => {
  const values = [
    {
      icon: <Award size={32} className="text-yutime-sage" />,
    ...
    {
      icon: <Wallet size={32} className="text-yutime-sage" />,
          ...
          {values.map((value, index) => (
            <div key={index} className="text-center space-y-6">
              <div className="flex justify-center">
                <div className="w-16 h-16 rounded-full bg-yutime-soft-sage/30 flex items-center justify-center">
                  {value.icon}
                </div>
              </div>
              <h3 className="text-xl font-bold text-yutime-forest">{value.title}</h3>
