    ...
    <section className="bg-yutime-softWhite py-16 md:py-24">
      <div className="container max-w-7xl mx-auto px-4">
          ...
          {values.map((value, index) => (
            <div key={index} className="text-center space-y-6 group">
              <div className="flex justify-center">
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-yutime-warmCream to-yutime-lightCoral flex items-center justify-center shadow-soft group-hover:shadow-warm transition-all duration-300 hover-lift">
                  {value.icon}
                </div>
              </div>
              <h3 className="text-xl font-semibold text-yutime-coral font-heading">{value.title}</h3>
              <p className="text-yutime-richBrown leading-relaxed">{value.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-yutime-coral/10 to-yutime-coral/10 px-6 py-3 rounded-full border border-yutime-coral/20">
            <span className="text-2xl">🌱</span>
            <span className="text-yutime-coral font-medium">Every expert was once a beginner</span>
          </div>
        </div>
      </div>
    </section>
