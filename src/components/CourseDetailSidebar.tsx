...
const CourseDetailSidebar = ({ course, onClose }: CourseDetailSidebarProps) => {
  return (
    <div className="p-6 bg-yutime-cream min-h-full">
      <button 
        onClick={onClose}
      ...
      <div className="space-y-8">
        ...
        <div className="relative h-48 md:h-64 rounded-2xl overflow-hidden shadow-warm hover-lift">
          ...
          {/* Encouraging badges */}
          <div className="absolute top-4 left-4 bg-yutime-sunshine text-yutime-sage px-3 py-1 rounded-full text-xs font-medium shadow-soft">
            ✨ Popular Choice
          </div>
        </div>
        ...
        <div className="card-warm shadow-warm">
          ...
          <div className="space-y-4 mb-6">
            <Button className={`w-full py-3 text-white rounded-xl font-medium transition-all duration-300 transform hover:scale-105 ${
              course.isPurchased 
                ? 'bg-yutime-sage hover:bg-yutime-sage/90 shadow-soft' 
                : 'bg-yutime-coral hover:bg-yutime-coral/90 shadow-warm'
            }`}>
              {course.isPurchased ? '🎯 Continue Learning' : '🚀 Start Your Journey'}
            </Button>
            {!course.isPurchased && (
              <Button variant="outline" className="w-full py-3 rounded-xl border-yutime-sage/30 text-yutime-sage hover:bg-yutime-sage/5 font-medium">
                👀 Try Free Preview
              </Button>
            )}
          </div>

          <div className="space-y-3 text-sm border-t border-yutime-sand_dark pt-6">
            <div className="flex justify-between items-center">
              <span className="text-yutime-warmGray">Level:</span>
              <span className="font-medium text-yutime-sage bg-yutime-sage/10 px-2 py-1 rounded-lg">{course.level}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-yutime-warmGray">Lessons:</span>
              <span className="font-medium text-yutime-sage">{course.lessons} chapters</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-yutime-warmGray">Duration:</span>
              <span className="font-medium text-yutime-sage">{course.totalTime}</span>
            </div>
            {course.instructor && (
              <div className="flex justify-between items-center">
                <span className="text-yutime-warmGray">Guide:</span>
                <span className="font-medium text-yutime-sage">{course.instructor}</span>
              </div>
            )}
          </div>
        </div>
        ...
        {course.curriculum && (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-yutime-sage">Your Learning Path</h2>
            <div className="space-y-3">
              {course.curriculum.map((lesson, index) => (
                <div key={index} className="flex items-start space-x-3 p-4 bg-yutime-softWhite rounded-xl border border-yutime-sand hover-lift">
            ...
            {/* Encouraging message */}
            <div className="bg-gradient-to-r from-yutime-sage/5 to-yutime-coral/5 p-4 rounded-xl border border-yutime-sage/20">
              <div className="flex items-center space-x-2">
                <span className="text-2xl">🌟</span>
                <div>
                  <p className="text-yutime-sage font-medium text-sm">You're capable of amazing growth!</p>
                  <p className="text-yutime-warmGray text-xs">Take it one lesson at a time</p>
                </div>
              </div>
            </div>
          </div>
