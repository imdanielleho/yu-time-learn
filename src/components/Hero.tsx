...
const Hero = () => {
  return (
    <section className="bg-gradient-to-b from-white to-yutime-white py-16 md:py-20">
      <div className="container">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6 animate-fade-in">
            <h1 className="text-yutime-charcoal leading-tight">
              Learn New Skills at <span className="text-yutime-purple">Your Pace</span>
            </h1>
            ...
            <div className="pt-4 flex items-center text-yutime-charcoal">
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="w-10 h-10 rounded-full bg-yutime-purple/20 border-2 border-white flex items-center justify-center text-sm font-medium">
                    {i}
                  </div>
                ))}
              </div>
          ...
          <div className="relative h-80 md:h-96 lg:h-[500px] rounded-2xl overflow-hidden shadow-card animate-fade-in">
            <div className="absolute inset-0 bg-yutime-gold/10 z-10 rounded-2xl"></div>
            <img 
